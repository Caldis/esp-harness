...
/* NOTE: removed `driver/ledc.h` — was an unused include that broke under
 * ESP-IDF v6.0 component reorganisation (ledc moved out of the umbrella
 * `driver` component). The BSP source contains zero ledc_* references. */
...
static lv_display_t *bsp_display_lcd_init(const bsp_display_cfg_t *cfg)
{
    ...
    /* NOTE: use_psram changed false (was true upstream). PSRAM-backed
     * draw buffers on ESP32-S3 with CO5300 QSPI cause "DMA TX underflow"
     * during the initial full-screen render — GDMA can't pull from PSRAM
     * fast enough to feed the SPI3 output clock, FIFO empties, LVGL flush
     * task wedges in retry. With a 50-row buffer (480*50*2 = 48 KB) and
     * double-buffering enabled (96 KB total), internal SRAM has more
     * than enough room. ESP32-S3 has 512 KB. */
    esp_lv_adapter_display_config_t disp_cfg = {
        ...
        .profile = {
            ...
            .buffer_height = 28,               /* was 50; halved to ~26 KB
                                                * double-buffered so internal
                                                * SRAM has room for WiFi+BLE
                                                * static buffers when both
                                                * radios are enabled. */
            .use_psram = false,                /* SRAM, not PSRAM — DMA path */
